...
static int __dsp_system_binary_copy(struct dsp_system *sys, void *list)
{
	int ret;
	...
	if (bin->size > pmem->size) {
		...
		dsp_err("binary(%s) size is over(%u/%zu)\n",
				DSP_FW_NAME, bin->size, pmem->size);
		goto p_err;
	}
	ret = copy_from_user(pmem->bac_kvaddr, (void *)bin->vaddr, bin->size);
	if (ret) {
	...
	if (bin->size > pmem->size) {
		ret = -EINVAL;
		dsp_err("binary(%s) size is over(%u/%zu)\n",
				DSP_IVP_PM_NAME, bin->size, pmem->size);
		goto p_err;
	}
	ret = copy_from_user(pmem->kvaddr, (void *)bin->vaddr, bin->size);
	if (ret) {
		dsp_err("Failed to copy binary(%s)\n", DSP_IVP_PM_NAME);
		goto p_err;
	}
	...
	if (bin->size > pmem->size) {
		ret = -EINVAL;
		dsp_err("binary(%s) size is over(%u/%zu)\n",
				DSP_IVP_DM_NAME, bin->size, pmem->size);
		goto p_err;
	}
	ret = copy_from_user(pmem->kvaddr, (void *)bin->vaddr, bin->size);
	if (ret) {
	...
	if (bin->size > pmem->size) {
		ret = -EINVAL;
		dsp_err("binary(%s) size is over(%u/%zu)\n",
				DSP_IAC_DM_NAME, bin->size, pmem->size);
		goto p_err;
	}
	ret = copy_from_user(pmem->kvaddr, (void *)bin->vaddr, bin->size);
	if (ret) {
...
int dsp_system_start(struct dsp_system *sys, void *bin_list)
{
	...
	if (bin_list)
		ret = __dsp_system_binary_copy(sys, bin_list);
	else
