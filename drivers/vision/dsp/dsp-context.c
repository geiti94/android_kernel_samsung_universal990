		...
		struct dsp_ioc_boot_direct *args)
{
	...
	ret = copy_from_user(bin_list, (void *)args->bin_list_addr,
			args->bin_list_size);
	if (ret) {
		...
		void *ginfo, void *kernel_name, void *bin_list)
{
	...
	ret = copy_from_user(bin_list, (void *)load->bin_list_addr,
			load->bin_list_size);
	if (ret) {
		...
		struct dsp_ioc_control *control, union dsp_control *cmd)
{
	...
	case DSP_CONTROL_DISABLE_DVFS:
		...
		ret = copy_from_user(&cmd->dvfs.pm_qos, (void *)control->addr,
				control->size);
		if (ret) {
