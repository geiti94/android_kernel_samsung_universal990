...
second_wait:
    ...
    if(!gpio_get_value(sr100_dev->irq_gpio)){
      pr_err("IRQ Flag corrupted go back to Wait state......");
      goto second_wait;
    }
    sr100_disable_irq(sr100_dev);
    sr100_set_irq_flag(sr100_dev);
  ...
  if(!IsFwDnldModeEnabled) {
    ...
    if (ret < 0) {
      pr_info("sr100_dev_read 2: spi read error.. %d\n ", ret);
      goto fail;
    }
  ...
  return ret;
fail:
  pr_err("Error sr100_dev_read ret %d Exit\n", ret);
  if(!IsFwDnldModeEnabled){
    gpio_set_value(sr100_dev->ri_gpio, 0);
  }
  Abort_ReadPending = false;
  set_sync_flag_and_wakeup(true,sr100_dev);
  pr_err("Read Exit fail case");
  return ret;
}
